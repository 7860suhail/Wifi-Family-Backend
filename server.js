...
let deviceStatus = {
    ...
    deviceModel: "Android Device (Pixel 7)"
};

const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: 'apka.email@gmail.com',       // Apni Gmail ID dalein
        pass: 'apka_gmail_app_password'     // Gmail App Password dalein
    }
});
...
app.get('/api/pictures', (req, res) => {
    fs.readdir(uploadDir, (filesErr, files) => {
        if (filesErr) {
            return res.status(500).json({ success: false, pictures: [] });
        }
        const pictureList = files.map(file => ({
            filename: file,
            url: `https://wifi-family-backend.onrender.com/uploads/${file}`
        }));
        res.json({ success: true, pictures: pictureList });
    });
});

app.post('/api/update-status', (req, res) => {
    const { battery, location, deviceModel } = req.body;
    if (battery) deviceStatus.battery = battery;
    if (location) deviceStatus.location = location;
    if (deviceModel) deviceStatus.deviceModel = deviceModel;
